...
func (m *Manager) registerAndCreateGroup() {
	...
	for i := start; i < end; i++ {
		...
		go func(i int64) {
			...
			// 准备要发送的表单数据
			data := url.Values{}
			data.Set("phone_number", cast.ToString(i))
			...
			fmt.Println("注册返回值:", string(responseBody))
	...
	// 创建一个 POST 请求
	req, err := http.NewRequest("POST", httpURL+"/group", strings.NewReader(data.Encode()))
	if err != nil {
	...
	fmt.Println("创建群聊返回值:", string(responseBody))

	type respStrut2 struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data struct {
			Id string `json:"id"`
	...
	var respData respStrut2
	err = json.Unmarshal(responseBody, &respData)
	m.chatId = cast.ToInt64(respData.Data.Id)
	return
}
